...
class MyDslGenerator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		fsa.generateFile("State.java" , generateStateClass())
	...
	def generateStateClass() {
		'''
			public abstract class State{
				public String name;
				public boolean equals (Object o){
					if ( o instanceof State){
						return ((State)o).name == this.name;
					}
					return false;
				}
	...
	def private HashMap<State,State> getSrcToTarget(FSM fsm){
		...
		fsm.transition.forEach[t | result.put(t.source , t.target)]
		return result
	}
	
	def private String genrateMainClass (FSM fsm, HashMap<State, State> sourceToTarget){
		'''
		public class « fsm.name» {
			static String name = "« fsm.name»" ;
			static State initState = new «getInitState(fsm).name»();
			static State currentState = initState	;
			public static void main(String [] args){
				...
				while(!isFinished){					
					...
					«FOR k : sourceToTarget.keySet()»
						case "« k.name»" : 
							System.out.println("go from «k.name» to « sourceToTarget.get(k).name»");
							currentState = new «sourceToTarget.get(k).name»();
							« IF sourceToTarget.get(k).equals(getFinalState(fsm))»
								isFinished = true;
